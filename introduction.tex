...
Each of these components provide functionality to be used by other components, the system, or the user.
...
As components may require or conflict with other components, adding or removing a single component may have propagating effects across the system.
However, these technical aspects can be automated, and this automation allows users to change their component systems without the need for technical knowledge or expertise.
With little technical knowledge, the user relies entirely on the provided mechanisms to change their component system in a way that meets their requirements.

%how this change effects the object in question
\textit{How does changing a component system effect that system?}
Over time, as a component system is repeatedly changed by its user, it evolves through the \textbf{component system evolution} (CSE) process.
This evolution has not been explored indepth previously, 
and investigating what effects it has on the system is the focus of this thesis.
Through understanding how component systems evolve can inform users and developers about the effects of their choices. 
...
Ubuntu systems can be changed using \texttt{apt-get} \citep{Barth2005}, and Eclipse systems can be changed  altered using and can be changed by adding, removing and upgrading components.
