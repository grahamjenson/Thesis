...
In chapter \ref{formal} the evolutionary step function is defined as taking an evolution problem ($EP$), an order over component systems to define optimality ($<_{\alpha}$), 
and returns an optimal solution to the evolution problem.
...
A common way, in both academia \citep{abate2011} and industry \citep{leBerre2010}, 
in which to implement CDR is to map the evolution problem to a Boolean Satisfiability Problem (SAT),
and find optimal solutions through pseudo-Boolean optimisation (PBO) \citep{dixon2004automating}.
This allows the use of robust and fast solvers like SAT4J \citep{Berre2008}, to efficiently find solutions to the potentially difficult evolution problems. 

%%%In this chapter\ldots
In this chapter the CDR implementation of GJ solver is presented.
Firstly in section \ref{impl.SAT}, SAT solvers are discussed as they are the core of many current CDR implementation. 
This discussion includes recent advancements to SAT solver technology, including its extension to handle pseudo-Boolean constraints.

The CDR implementation, called GJSolver, created through this research is then presented in section \ref{impl.gjsolver}.
...
The extension of DPLL with pseudo-Boolean constraints is discussed, as it allows for problems to be more efficiently represented.
...
That is,
\begin{defs}
Given a tuples of literals $lits = \langle l_1,\ldots,l_n \rangle$ and a tuple of coefficients $coeffs = \langle a_1,\ldots,a_n\rangle$,
a pseudo-Boolean function takes a set of literals $x$, and is defined as

$f(x) = \sum \limits_{l_i \in (lits \cap x)} a_i$

A pseudo-Boolean function can be represented by a tuple $\langle lits, coeffs \rangle$
\end{defs}
For example, the pseudo-Boolean function $f$ defined as  $\langle \langle x_1, \neg x_2\rangle, \langle 1,3\rangle \rangle$,
will equal $f(\{x_1,\neg x_2,x_3\}) = 4$.
...
A pseudo-Boolean constraint can be represented by a tuple $\langle f,  ineq,  k\rangle$ where $ineq \in \{\geq,>,\leq,<,=\}$.
\end{defs}

For example, the pseudo-Boolean function $f$ defined above defined with can be used to create the constraint $f(x) > 2$.
This constraint will be satisfied with the input $f(\{ x_1, \neg x_2 \}) = 4$, but not with the input $f(\{ x_1,  x_2 \}) = 1$.

pseudo-Boolean constraints, can be translated into the standard CNF, but the original pseudo-Boolean representation has been shown to be exponentially more concise \citep{dixon2004automating}.
...
In this section the initial requirements for GJSolver are discussed, this includes a description of the requirements for entering the MISC competition.
A formal mapping to the evolutionary step function defined in chapter \ref{formal} is given, where the evolution problem is defined using SAT and PB constraints,
and finding an optimal solution is defined using pseudo-Boolean optimisation.
The algorithm that are used to find an optimal solution are then presented, and the drawbacks of this approach are discussed.
Finally, the verification of this solver through the MISC is discussed, and the results that it gained are presented and analysed.
...
In this section the evolution problem is mapped to a series of SAT and PB clauses, and the optimisation order and criteria are mapped to a PBO problem. 

The order is represented as a criteria, which is defined as a ranking function and order, that can be extracted from MOF.

A component system is a set of components, and in the SAT and PB constraints a set of literals is defined.
To map between the two, given a component system $\alpha$:
\begin{itemize}
  \item $\alpha := \{x \mid x \in \alpha\} \cup \{\neg x \mid x \in P \wedge x \not \in \alpha\}$
\end{itemize}
That is, a component system is mapped to the set of literals where all components included in the system are positive literals, and all literals not in the system are negative. 
...
\begin{enumerate}
  ...
  \item $a_1 + \ldots + a_n = 1 := 1.a_1 + \ldots + 1.a_n = 1$  
\end{enumerate}

By extracting an evolution problem from a CUDF document, and mapping the constraint as such, a solver such as SAT4J, can be used to find a solution.
...
while the assignments should also satisfy all other constraints. 

That is, given a pseudo-Boolean function $f(x)$, pseudo-Boolean optimisation is meant to find an assignment of $x$, where $f(x)$ is maximum or minimum.
Such a pseudo-Boolean function can be mapped to a criterion in order to be used in our formal framework.
However, there is one problem, the PB function only takes the proposed system as an argument where the rank function takes both current and proposed systems.
A pseudo-Boolean function is defined with respect to the current solution such that:
\begin{defs}
A pseudo-Boolean criteria is defined as either $\langle f_{\alpha}(\beta), <$ or $\langle f_{\alpha}(\beta), >$,
to maximise or minimise $f_{\alpha}(\beta)$, if $f_{\alpha}(\beta) = rank(\alpha,\beta)$.
\end{defs}

To demonstrate this PBO, the criteria to minimise the amount of components added to the system is defined.
The function $rank^{added}$ is defined to return the amount of components in the proposed system that are not in the previous system (the added components),
i.e. $rank^{added}(\alpha,\beta) = |[x \mid x \in \mathbb{C} \wedge \neg x \in \alpha \wedge x \in \beta]|$.
Given pseudo-Boolean functions only measure the proposed system,
the function $f_{\alpha}(\beta)$ is defined by summing all literals that are negative in the current system and positive in the proposed system;
i.e. given the set of variables $\{x_1,\ldots,x_n\} = [x \mid x \in \mathbb{C} \wedge \neg x \in \alpha]$, the pseudo-Boolean function is defined $f_{\alpha}(\beta) =  1.x_1 + \ldots +  1.x_n$.
