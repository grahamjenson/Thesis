...
Calculating all the alterations that occur during evolution is described as \textbf{resolving} a \modelname instance.

Resolving a \modelname instance can require significant computational effort due to the complexity of the problem.
...
\modelimpl is an optimisation problem
where the variables are Boolean, the constraints are a SAT formula extended with pseudo-Boolean (PB) constraints (SAT+PB) \citep{dixon2004automating},
and optimisation criteria are lexicographic and defined with PB functions.
...
Some criteria used by the LIS algorithm to resolve \modelname instance are also presented in this chapter. 
These criteria are given MOF strings, and mapped to \modelname criteria.
...
This section describes the lexicographic iterative strengthening algorithms to solve \modelimpl problems.
...
Both 'paranoid' and 'trendy' have pre-defined criteria defined in MOF, e.g. 'paranoid' tracks criteria in MOF is \texttt{-removed,-changed} (these are defined above).
