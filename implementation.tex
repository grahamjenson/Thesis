...
Through extending DPLL to be able to handle such constraints, some problems can be more easily expressed and solved.
...
A pseudo-Boolean constraint is then an inequality relating the function to a number, e.g. $f(x) \leq k$.

This is a broad definition of these concepts, 
to be efficiently included into the DPLL algorithm the pseudo-Boolean representation has been restricted to natural numbers and a linear equation.
That is,
\begin{defs}
...
To define a psuedo-Boolean function the previous system is used to define the function, and the proposed system is used as input to the function.
This makes the ranking functions higher order functions, they return a pseudo-Boolean equation to be maximised or minimised.

For example, the criteria to minimise the amount of components added to the system can be defined in such a manner.
The function $Rank^{added}$ must then be defined to return a pseudo-Boolean function that measures the amount of added components.
To calculate the amount of added components all variables that are components, and are not in the initial solution should be added in the function.
Therefore, $Rank^{added}(l_1,l_2)$ returns a pseudo-Boolean function $f$ defined as a set of pairs such that $f(l_2) = [(1, x) \mid x \in \mathbb{C} \wedge \neg x \in l_1]$.
That is, all components that are not in the original system, but are in the proposed solution are counted. 
This function can then be used to search for a minimal amount of added components.
