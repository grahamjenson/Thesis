...
The previous chapter presented the model \modelname whose instances describe evolutions of component systems as a series of evolution steps.
Each step evolves the component system to a new system that satisfies the user's request and system constraints, and is optimal with respect to some criteria.
The process of calculating the resulting systems of the evolution steps is described as \textbf{resolving} a \modelname instance.
...
This chapter describes the relationships between an instance of \modelname and \modelimpl problems.
A \modelimpl problem is defined as a Boolean satisfiability (SAT) problem extended with pseudo-Boolean (PB) constraints \citep{dixon2004automating},
and lexicographically ordered pseudo-Boolean criteria. 
...
\section{\modelimpl}
\label{impl.mapping}
A \modelname instance can consist of many evolution steps.
Each of these steps consists of an evolution problem, and an evolution preference order that is defined as the lexicographic composition of criteria.
...
An instance of \modelimpl consists of:
\begin{itemize}
...
First an optimal solution is defined with respect to only one criterion:
\begin{defs}
Given a formula $F$, and a pseudo-Boolean criterion $\langle f,R, I \rangle$, 
a set of literals $P$ is an optimal solution w.r.t. $\langle f,R, I \rangle$ iff it is a solution to $F \cup I$ and $F \cup I \cup \{\langle f, R, f(P)\rangle\}$ is unsatisfiable. 
\end{defs}
By adding the psuedo-Boolean constraint $\langle f, R, f(P)\rangle$ to the formula it ensures that there is no better solution available. 
The optimal solution is then one where there is no better solution to $F \cup I$

First an optimal solution given two PB criteria:
\begin{defs}
Given a formula $F$, and two pseudo-Boolean criteria $\langle \langle f_1,R_1, I_1 \rangle,\langle f_2,R_2, I_2 \rangle \rangle$, 
a set of literals $P$ is an optimal solution w.r.t.  $\langle \langle f_1,R_1, I_1 \rangle,\langle f_2,R_2, I_2 \rangle \rangle$  
iff $P$ is an optimal solution w.r.t. to $\langle f_1,R_1, I_1 \rangle$,
and $F \cup I_1 \cup I_2 \cup  \{\langle f_1, =, f_1(P)\rangle\}  \cup \{\langle f_2, R_2, f_2(P)\rangle\}$ is unsatisfiable. 
\end{defs}

\begin{defs}
Given a \modelimpl instance with SAT+PB formula $F$, 
and PB criteria $\langle \mathfrak{crit}_1,\ldots,\mathfrak{crit}_n \rangle$ where $\mathfrak{crit}_i = \langle f_i, R_i , I_i \rangle$,
a solution to \modelimpl is a solution to $F \cup I_1 \cup \ldots \cup I_n$.  
\end{defs}
A solution to the 
