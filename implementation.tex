...
This chapter describes the resolving of a \modelname instance by mapping each of its steps to, then solving, a \modelimpl problem.
...
GJSolver takes a CUDF* document parses it into a a \modelname instance (as described in appendix \ref{apx.cudf}),
and resolves it with the process described in this chapter.
...
Consider a SAT problem where $V = \{a,b,c\}$ and $F = \{\mathcal{C}_1,\mathcal{C}_2\}$, where clauses $\mathcal{C}_1 = \{a,b\}$ and $\mathcal{C}_2 = \{\neg b, c\}$.
...
Some such amendments are described in \citep{Sheini2006}, 

\subsection{\modelimpl Problem}
\label{impl.lexsatproblem}
In order to define a \modelimpl problem, criteria that define an optimal solution are required.
The pseudo-Boolean criteria use a PB function to define an optimal solution.
...
This is to ensure that the auxiliary variables required by the criteria are available.
...
For each $i$ between $1$ and $n$ there is an evolution step.
...
Of the criteria that can be mapped to PB criteria, generalizing their mapping may be a difficult and unrewarding challenge.
...
Therefore, only specific set of criteria are mapped.
