...
This was because each package had a set of constraints describing compositions that it would be functional in,
and satisfying these was a complex task.  
Furthermore, satisfying these constraints was difficult because there was a selection problem (as discussed above), 
where there could be many possible solutions to select from.
...
This lowering of the technical knowledge required to become a composer of a component system has had other effects discussed in the next section.
...
Firstly, for these component models to conform to the previously described definition, their components must describe their constraints.
...
However, for the component to be functional the service layer constraints must be considered.
...
Therefore, a number of frameworks have emerged that define service constraints, e.g
...
These constraints can have cardinalities, e.g. a text editor can use multiple spell checkers, and other programmatic features.
...
as it contains explicit constraints, and the programmatic evolution requirements necessary for CDR to be used.
...
Maven both explicitly declares constraints and provides an interface to evolve its component systems. 
...
Each of these component models provide an explicit description of their components constraints, and a mechanism to alter a component systems composition.
These constraints fall into two groups:
\begin{enumerate}
...
OBR, Maven, and SOFA allowed the definition of a type of dependency 
that allowed them to express additional information about constraints. 
