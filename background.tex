...
Software evolution \citep{lehman1980} is the process of repeated change made to a software system to maintain it, or to extend its functionality over the systems lifetime.
...
That is software maintenance is a range of activities through which software is changed, and software evolution is the process of repeatedly changing the system; this is similar to the definition given in \citep{lehman1980}.
...
Composers are also responsible for changing their systems, as they have the power to alter the composition of components that make the system.
As a composer make changes their component system over time, the component system is said to evolve.
This relationship between component system change and CSE is a similar relationship between software maintenance and software evolution.
The change of a component system is an activity performed by the composer, and a component system evolves over many such changes.  
 
%%%In this chapter\ldots
This chapter illustrates the histories and presents the current picture of software evolution, component systems, and CSE.
...
These definitions are made with respect to CSE, and leave superfluous detail undefined.
...
The foundations for software evolution, software components, and component systems were laid in the late 1960's.
...
This section briefly describes these advancements, building up to a discussion about CSE.
...
This realisation, that software requires significant expense to maintain, lead researches to study how software changed after deployment.
...
The second study by \cite{lehman1980} explored how the evolution affected software.
...
Software evolution is still seen at a young field \citep{Godfrey2008} as many open questions remain to be answered.
...
\end{itemize}
\cite{Yourdon1976} lists the goals of structured design as efficiency, maintainability, modifiability, generality, flexibility, and utility.
...
\cite{PrietoDiaz1987} describes this as the \textbf{selection problem}, where a composer can have many alternative compositions of components to select from.
...
Current research into the selection problem looks at the fundamental questions:
\begin{itemize}
...
Additionally, the original concept of a software component subindustry (described by \cite{McIlroy1969}) is seen to of never truly come to fruition \citep{Szyperski2002}.
...
This philosophy leads to the first two rules (out of fifteen) of Unix \citep{raymond2003art}:
\begin{itemize}
...
Initially, a significant amount of technical expertise was required to change the composition of packages to in a Debian system.
This was because the selection problem (as discussed above) had to be resolved by the person changing the system.
...
With the release of the application \textit{apt-get} in the late 1990's, the necessary technical knowledge to compose a Debian system was significantly reduced.
...
This automated the search for a solution to the selection problem which had restricted the evolution Debian systems prior to apt-get's release.
apt-get (and applications like it) have been called the ``single biggest advancement Linux has brought to the industry'' 
...
This lowering of the technical knowledge required to become a composer of a component system by solving the selection problem has had other effects discussed in the next section.
...
However, with applications like apt-get (discussed in the previous section) 
the user has started to fill the role of the composer for their systems.
...
A system is also more fragile, as the quality of a system may not be verifiable by a potentially non-technical user.

When the user is the composer of their system, it also breaks apart two important processes,
component evolution and CSE.
The developer, no longer the composer of the component system, is now solely in charge of evolution of the components they develop and maintain. 
...
This maintenance requires technical knowledge of the internal workings and structure of the component, therefore must be accomplished by a developer.
Therefore, component evolution is  similar to software evolution, where it is a process enacted by a developer.
However, a significant difference from software evolution occurs because there are many systems that a component could be deployed in,
and testing each one to ensure the components functionality is practically impossible.

To solve this problem, constraints are included with the component that describe compositions that it will be functional in.
These constraints could describe conflicts between components, and/or dependencies on other components.
...
Such component states are tagged with \textit{version}s, this provides an order to the evolution of a component.
Unlike software evolution, component evolution depends more on the version of components as they are often used in the constraints that describe valid compositions.
...
This is opposed to the evolution of individual component, where a developer alters a component and its constraints.
...
If an application, like apt-get, is available these actions will not require much technical knowledge.
This application can use the constraints described by the components to select a  valid system that satisfies a users request.
...
A problem arises when trying to measure the evolved state of a component system.
A component system is a set of components, where each component can different  versions.
...
For example, a system that has version $1$ of component $a$ and version $2$ of component $b$, is neither more or less evolved than a system with version $2$ of $a$ and version $1$ of $b$.
