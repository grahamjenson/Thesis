...
A component in the SOFA framework is defined by it's frame, which is the core definition of what the component requires and provides.
Like Fractal, SOFA is a recursive component model, allowing for a composite of components to be treated as a component.
...
This meta-data descirbes what a component using the tag \verb+frame+.
The sub elements \verb+requires+ and \verb+provides+ describe what the component requires and provides through an external interface definition with the namespace \verb+sofatype://nz.geek.maori.spellChecker+.
...
it only allows for controlled evolution; evolution that conforms to specified evolution patterns.
Other component models allow the change of the component system on a fine grained level of adding or removing a component, where this model has only allows to change the component system.
This is meant to increase the manageability and predictability of a component systems evolution. 
