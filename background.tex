...
There can be a system where user involvement is required to add or remove a component, one where installing a component required manual configuration.
These systems could not use an algorithm like CDR to change their system, therefore would not be a component in this research.
...
These relationships can have cardinalities, e.g. a text editor can use multiple spell checkers, and other programatic features.
...
To interact with the component framework to install and remove bundles and their containing services, a console was implemented.
This console give users interactive ability that all bundles have programatically.
A extract from the help text of this console is represented in figure \ref{osgiconsole}.
...
This console has the ability to add and remove bundles from the framework, as well as assign it to various life cycle states.
This gives complete evolution control for an OSGi system, with the ability to make changes without shutting the system down.
...
%%%What is Eclipse
Eclipse is a popular IDE and frameowrk

%%%It is like DS built on top of OSGi
Like the declaritive services framework built on top of OSGi, the Eclipse

%%%It uses the idea of extensions and extension points
...
The installation of a plugin into Eclipse can be accomplished by droping the plugin into the ``plugins'' folder in the appilcation directory.
...
Adding a component is done only during the compilation stage of Maven.
