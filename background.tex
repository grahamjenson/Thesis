...
Both the study from \cite{Lientz1980} and the laws from \cite{lehman1980} argue that
...
and refactoring tools \citep{fowler1999refactoring, Murphy-Hill2008} have been developed to restructure code to decrease complexity and increase maintainability. 
...
by describing the idea of a software components subindustry which created components to be used in software.
...
A problem soon emerged as the number of software components grew,  where the best set of components that satisfy the requirements of the system must be selected.
...
This automated the search for a satisfactory solution, solving the problem which had restricted the evolution of Debian systems prior to \texttt{apt-get}'s release.
The \texttt{apt-get} package manager and similar applications have been called the ``single biggest advancement Linux has brought to the industry'' \citep{Murdock2007}
...
A proper methodology for the development and evolution of software components is still being sought \citep{Szyperski2002}.
...
A system that provides CDR functionality would try to find a valid system that has up-to-date versions of a text-editor and a spell-checker.
...
CDR systems typically provide similar functionality that allow the user to \textbf{install} and \textbf{remove} components, adding or removing a component to or from the system.
Furthermore, typically provided is an \textbf{upgrade} function that removes then installs a higher version of the same component.
...
This may make a component system impossible to \textit{version} in a way similar to how individual components are versioned.
...
who look at the first ten releases of Debian and compare it to the evolution within biology.
...
This thesis contributes a formal model of component system evolution in chapter \ref{formal}.
...
They can be physical, as in electrical or mechanical components, or virtual, as in software components.
...
As such, many different component models have been developed, each targeting various domains with different functionality and technical aspects.
...
In this thesis a software component is defined as a unit of independent deployment, and third party composition,
...
It has implementations from organizations like the Eclipse Foundation with their Equinox framework \citep{mcaffer2010osgi}, 
...
The OSGi framework separates the components for deployment and run-time into bundles and services respectively.
...
\texttt{Import-Package} describes the dependence on packages provided by other bundles.
...
The \texttt{service} tag describes the services provided, and the \texttt{reference} tag expresses a dependence on another service.
...
OBR has been seen as a solution to simplify deployment of OSGi applications \citep{Jung2007}, 
...
\begin{enumerate}
  ...
  \item Minimise changing an installed component if unrelated to the request being made.
\end{enumerate} 
...
(each interface is in the package \texttt{org.objectweb.fractal.api}):

\begin{itemize}
  \item To create a component: \\ \texttt{factory.GenericFactory}\verb+#+\texttt{newFcInstance}.
  \item To add a component to a composite: \\ \texttt{api.control.ContentController}\verb+#+\texttt{addFcSubComponent}.
  \item To remove a component from a composite: \\ \texttt{control.ContentController}\verb+#+\texttt{removeFcSubComponent}.
\end{itemize}
...
Another aspect that will make changing Fractal systems difficult, is the lack of versioning information.
...
This model defines the component's namespace using the \texttt{groupId} tag, the name using the tag \texttt{artifactId}, and the version.
...
The text editor states using the \texttt{Depends} key that it requires a spellChecker to be installed, 
...
The implementation from Mancoosi is the Modular Package Manager (MPM) \citep{abate2011}.
...
\end{tabular}
\caption[Summary of presented component models' constraints.]{Summary of presented component models' constraints. Note: \textit{typed} refers to the ability to define the type of requirement.}
