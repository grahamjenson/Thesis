...
\end{tabular}
\caption{Mapping of the change, removed, new, one version per package, up-to-date distance and stable version criteria between MOF, \modelname and PB}
\label{apx.crittable}
\end{table}
...
\section{-stableversion(d)}
The stable version criterion is a modification of the uptodate distance criterion.
The only modification necessary is to the integers in the $f_{utdd}$, to create the $f_{sv}$ function using the stable version function described in definition \ref{exp.stablefunction}.
\begin{defs}
The function $stableweight$ is defined to return the $uptodatedistance$ of a component if it is stable, or $1000$ plus its $uptodatedistance$ if it is not,
i.e. $stableweight(c,d,\mathbb{C}_t) = \begin{cases} 1000 + uptodatedistance(c,\mathbb{C}_t)& \text{not }stable(c,d,t) \\ uptodatedistance(c,\mathbb{C}_t) & \text{otherwise} \\  \end{cases}$
\end{defs}

\begin{defs}
Given a number of days $d$, a set of components $\mathbb{C}_t$ the PB criterion for the stable version criteria is defined as $\langle f_{sv}, <, I_{sv} \rangle$ where 
$f_{sv}$ is defined with the literals $\{l_1,\ldots,l_i\} = \mathbb{C}_t$, and integers $\langle stableweight(l_1,\mathbb{C}_t), \ldots ,stableweight(l_i,\mathbb{C}_t)\rangle$.
The formula $I_{sv}$ then equals $\emptyset$.
\end{defs}
