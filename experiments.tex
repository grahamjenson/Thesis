...
This change can be reduced by creating a criterion that does not upgrade to a package until it has become \text{stable}.
This criterion is called \textit{stable version}.
To check exactly when a package is stable the $stable$ function is used.
\begin{defs}
The function $stable$ takes a component $c$, a number of days $d$, and a time $t$ and returns true iff:
\begin{enumerate}
...
This additional weight of 1000 is added only to encourage the system not install unstable components.
...
The reduction in change for the ``7 Days SV Upgrade'' user is the 23  changes over the year.
...
The reduction in change for the other users are: 29 for ``14 Days SV Upgrade'', 41 for ``21 Days SV Upgrade'', and 59 for ``28 Days SV Upgrade''.

There is a minor reduction in change when using this stable version criterion.
However, it makes a system out-of-date as the system continually waits for components to become stable.
