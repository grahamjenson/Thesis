...
The function $rank$ is defined to take the previous component system and proposed system and return a measure of optimality to be maximised or minimised. 
For example, given the current component system $\alpha$ and a proposed systems $\beta_1$ and $\beta_2$, 
if $rank$ is to be maximised, and $rank(\alpha,\beta_1) > rank(\alpha,\beta_2)$\footnote{the range of $rank$ (e.g. $\mathbb{R}$ or $\mathbb{N}$) and the relationship $>$ are left undefined here},
...
Given the previous system $\alpha$, the constraints $R \cup \Delta$ and the function $rank$ to be maximised the function $E$ is defined as
...
If $rank$ is to be minimised the function is defined with $\min$ instead of $\max$.
...
Finding an optimal solution by maximising or minimising the $rank$ function over the set of possible solutions of an evolution problem is NP-Hard.
...
\section{Multi-criteria Optimisation}
\label{formal.opt}
