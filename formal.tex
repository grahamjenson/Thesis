...
The syntax of the preamble property value is defined in the BNF grammar as \verb+Properties+ in figure \ref{formal.cudfpropertysyntax}.
...
Each of these other possible types is defined in the same way as the  \verb+int+ type, however their semantics differ significantly.

\subsubsection{Components}
Each component is defined by a name and version, as described in section \ref{formal.step}, this is extracted from the package descriptions in a CUDF document.
...
Given the set of package descriptions in a CUDF document, $PD$, the set of components is defined where $\mathbb{C}$ equals $[ c(pd) \mid pd \in PD]$.

To refer to a property assigned to a component $a$, of the key \verb+key+, it is written $a$.\verb+key+.
That is, $a$.\verb+key+ $= property(pd, $\verb+key+$)$ where $a = c(pd)$.
...
A CUDF document not only described the set of all components, but the set of components that defines the current system.
...
This final caveat is directly taken from the DPKG specification, as it allows a package to declare a conflict with itself and this forces similar packages to be removed if it is installed.  
...
An example of this CUDF format is presented in figure \ref{CUDFEXAMPLE}. 
...
In the document in figure \ref{CUDFEXAMPLE}, the preamble contains the addition of one property \verb+size+ of type \verb+int+.
...
such that $C = \{\langle syslib,1\rangle, \langle syslib,2\rangle, \langle textEditor,1\rangle, \langle spellChecker,1\rangle, \langle tpspeller,1\rangle\}$.
...
The initial system that $X$ includes all components whose package stanzas include the property \verb+installed+ set as \verb+true+ (if it is not defined it the property defaults to \verb+false+),
therefore $X = \{\langle syslib,1\rangle \}$.
...
The only depends constraints in this document are from the $\langle textEditor,1\rangle$ component;
...
which are mapped to $\langle $\verb+spellChecker+$,0,>\rangle$ and $\langle $\verb+spellCheckerService+$,0,>\rangle$ denoted as $pf_1$ and $pf_2$ respectivly.
The second has exactly one package formulae, mapped to $\langle $\verb+syslib+$,1,>\rangle$ denoted as $pf_3$.

The $packagesThatSatisfy(pf_1)$ equals $\{\langle spellChecker,1\rangle\}$, and $packagesThatSatisfy(pf_2)$ equals $\{\langle tpspeller,1\rangle\}$.
The function $dependsOn(\langle textEditor,1\rangle , \{pf_1,pf_2\} )$ 
then equals the constraint $\langle textEditor,1\rangle \rightarrow \langle spellChecker,1\rangle \vee \langle tpspeller,1\rangle$,
expressing the text editor depends on either the spell checker component or the third party spell checker service.

The $packagesThatSatisfy(pf_3)$ equals $\{\langle syslib,2\rangle\}$,
this means the function  $dependsOn(\langle textEditor,1\rangle , \{pf_3\} )$ returns the constraint  $\langle textEditor,1\rangle \rightarrow \langle syslib,2\rangle$.
This expresses that for the text editor to be installed the component $\langle syslib,2\rangle$ must be installed.

%%%Describe the conflicts constraints
In this example there are two components that declare conflicts, $\langle syslib,1\rangle$ and $\langle syslib,2\rangle$.
...
The function $packagesThatSatisfy(pf_4)$ equals $\{ \langle syslib,1\rangle, \langle syslib,2\rangle \}$,
...
Therefore, $conflictsWith(\langle syslib,1\rangle)$ returns the set of constraints $\{\langle syslib,1\rangle \rightarrow \neg \langle syslib,2\rangle\}$,
and $conflictsWith(\langle syslib,2\rangle)$ returns the set of constraints $\{\langle syslib,2\rangle \rightarrow \neg \langle syslib,1\rangle\}$.
...
$\{ \langle textEditor,1\rangle \rightarrow \langle spellChecker,1\rangle \vee \langle tpspeller,1\rangle,
\langle textEditor,1\rangle \rightarrow \langle syslib,2\rangle,
\langle syslib,2\rangle \rightarrow \neg \langle syslib,1\rangle, 
\langle syslib,2\rangle \rightarrow \neg \langle syslib,1\rangle\}$.
...
This package formula maps to $\langle $\verb+textEditor+$,0,>\rangle$, where $packagesThatSatisfy(\langle textEditor,0,>\rangle)$ equals $\{\langle textEditor,1\rangle\}$.
Through functions $install$ and $installpkgflist$ this returns the constraint $\langle textEditor,1\rangle$,
which asserts that the text editor component will be included in the new system.
This makes the set of request constraints $\Delta = \{\langle textEditor,1\rangle\}$.
...
Firstly, given the request ensures that the text editor component is included, the component $\langle textEditor,1\rangle$ will be installed.
This then asserts that the system library component $\langle syslib,2\rangle$ will be installed.
As the system library components conflict, $\langle syslib,1\rangle$ must not be installed.

Of the two components that are left, $\langle spellChecker,1\rangle$ and $\langle tpspeller,1\rangle$, at least one must be installed due to the dependencies of the text editor.
This gives three possible solutions to this problem, all include $\langle textEditor,1\rangle$ and $\langle syslib,2\rangle$, exclude $\langle syslib,1\rangle$,
and either include only $\langle spellChecker,1\rangle$ or $\langle tpspeller,1\rangle$, or both.

Of course only one system can be returned by the function $E_{R \cup \Delta}(X)$, therefore one of these three solutions must be selected given some optimisation criteria.
If that criteria specified to select a solution with the minimal \verb+size+ value, 
then looking at the component properties it can be seen that $\langle spellChecker,1\rangle$ has less size than $\langle tpspeller,1\rangle$.
Therefore, $E_{R \cup \Delta}(X)$ equals $\{\langle syslib,2\rangle, \langle textEditor,1\rangle, \langle spellChecker,1\rangle\}$.
...
However, for the purpose of the MISC competition, the Mancoosi organisation did give a specification to represent different optimisation criteria.
This representation was designed to be passed to the CUDF solver on the command line, this meant that it must be simple to express.
...
This specification does not express the possible product order of the problem, however it could be modified to do so.

%TODO change to size as in \sum
For example, given there are two criteria defined \verb+-change+ and \verb+-size+, which express the desire to minimise the change and size of the system.
The criterion \verb+-size+ is mapped to the $RS_{size} = \langle L_{size},D,V \rangle$ where  $L_{size} = \langle \mathbb{N}, \leq \rangle$,
and the ranking function which returns the size of the changed system, i.e.  $Rank_{size}(A,B) = \sum_{b \in B} property(pd,$\verb+size+) where $b = c(pd)$.

The criterion \verb+-change+ is mapped to the $RS_{change} = \langle L_{change},D,V \rangle$ where  $L_{change} = \langle \mathbb{N}, \leq \rangle$,
and the ranking function that measures the symmetric difference between the two systems, i.e. $Rank_{change}(A,B) = A \Delta B$.

If the optimisation criteria are defined as \verb+-size,-change+, then the two ranking systems are composed using the operator $\odot$,
such that $RS = \langle \langle \mathbb{N} \times \mathbb{N}, \leq_L \rangle, D, V \rangle$ where $\leq_L$ is a lexicographic order;
$(a,b) \leq_{L} (a',b')$ iff $a <_{1} a'$ or $(a = a'$ and $b \leq_{2} b')$.
Also, the ranking functions are combined such that $Rank(A,B) = \langle |B|, A \Delta B \rangle$.

This information completely defines the optimisation problem.
Given this definition, the example presented in \ref{formal.example} can be  
