...
To begin to study the component evolution process, first it must be modeled.
In section \ref{formal.step} component system evolution is modeled.
An evolution problem is defined as a set of constraints created by component relationships and a user request.
The process of component system evolution is caused by iteratively solving evolution problems, which is discussed in section  \ref{formal.evo}.

The way in which an optimal solution is selected to an evolution problem is described in this research through the use of a criteria composed lexicographically.
Each criterion is represented as a ranking system over a complete lattices, as described in section \ref{formal.opt}.
...
are used to describe the evolution problem and criteria by which to select an optimal solution.
...
%%%What a component is
The first step in formally defining component system evolution, is to define what is meant by component.
In definition \ref{formal.componentdef} components are defined as: 
\begin{defs}
\label{formal.componentdef}
$\mathbb{C}$ is the space of components, where each component is described by a name and version ($\langle n, v\rangle$), such that no two components can have the same name and version.
\end{defs}
A component can be abbreviated to $n^v$, where the version is superscript of the name e.g. $c^2$ is a component $\langle c, 2\rangle$.

%%%What a component system is and the relationships
A component system is defined in definition \ref{formal.componentsystemdef}:
\begin{defs}
...
To define the relationships between components, and the user request, constraints are used.
Definition \ref{formal.constraintdefs} describes a constraint:
\begin{defs}
...
\begin{defs}
A solution to an evolution problem is in the intersection of all its constraints, $s \in con_1 \cap \ldots \cap con_n$ where $EP = \{con_1,\ldots,con_n\}$
\end{defs}

As there may be many solutions to an evolution problem, and some solutions are more optimal than others.
An evolution can be seen as moving from one system to another, and in order to select an optimial evolution a binary relationships is defined as:
\begin{defs}
A binary relationship over pairs of component systems is defined such that $<: (2^{\mathbb{C}},2^{\mathbb{C}}) \times (2^{\mathbb{C}},2^{\mathbb{C}})$
\end{defs}
TODO
This function is defined as:
\begin{defs}
The evolutionary step function $E$ is defined such that is takes an evolution problem $EP$,
a previous system $\alpha$, and an order over pairs of component systems  
\end{defs}
