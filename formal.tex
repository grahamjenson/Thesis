...
Having such a model where concepts are defined will also enable discussion.
...
%%%What a component is
First what a component is must be defined.
\begin{defs}
...
A \textbf{component system} is a finite subset of components at a given time, $\alpha \subseteq_{finite} \mathbb{C}_t$. 
\end{defs}
The characters $\alpha,\beta,\gamma$ are used to denote component systems.

Constraints are used to express systems that are ``allowed''.
\begin{defs}
\label{formal.constraintdefs}
A \textbf{constraint} is a finite set of component systems, $con \subseteq_{finite} 2^{\mathbb{C}_t}$.
\end{defs}

Only certain types of constraints are allowed in this model.
\begin{defs}
The \textbf{types of constraints} are:
\begin{enumerate}
...
\begin{defs}
\label{formal.constraintdefs}
A component system \textbf{satisfies} a constraint, if the system is in the constraint, $\alpha \in 2^{\mathbb{C}}$.
\end{defs}
...
the \textbf{optimal} system is the maximum solution to the evolution problem, with respect to the evolution preference order.
\end{defs}

\begin{defs}
An \textbf{evolutionary step function}, $E:2^{2^{\mathbb{C}_t}}$, returns an optimal solution to an evolution problem, given an evolutionary preference order.
\end{defs}
...
This is accomplished by mapping the finding of a solution to an evolution problem to an NP-Complete problem,
called ``exactly one in three'', that is presented in \citep{Schaefer1978} as problem ``NP1''.
...
$S_1, \ldots, S_n$, equals a set of constraints where each can at most be satisfied by three component systems,
$T$ is the constraint that only one component can be installed in the system, $a_1 + \ldots a_n = 1$ where $\{a_1,\ldots,a_n\} = \mathbb{C}_t$,

Given a set of components $\mathbb{C}_t = \{a,b,c\}$, the constraint $a + b + c = 1$.
