...
This syntax shows that the depends syntax is a conjunction (separated by the delimiter ``,'') of dijunct (seperated by the delimiter ``|'') package formulas.
Therefore, the syntax in conjuctive normal form, where using the function $packagesThatSatisfy$ to create sets of packages the dependency constraints can be created.
Using the function $cnf$ which takes a component and uses the value of the components property \verb+depends+ to return a set of sets of package formula,
e.g. $property(c,$\verb+depends+$)$ equals \verb+a, b | c+ returns the set $\{\{a\},\{b,c\}\}$.
