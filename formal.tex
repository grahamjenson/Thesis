...
Given an evolutionary problem $\delta_t \cup \omega_t$, and the installed system $\alpha_{t_{i-1}}$, if $\delta_t \cup \omega_t$ is unsatisfiable, 
then $\epsilon(\alpha_{t_{i-1}},\delta_t \cup \omega_t) = \alpha_{t_{i-1}}$.
This solution is implemented in current solvers, like apt-get,
and when an evolution step fails some output is generated that highlights the conflicting constraints. 
...
The Common Upgradeability Description Format (CUDF) specification presented by \citep{treinen2009common} is used as the language to represent evolution problems.
...
The Mancoosi Optimisation Format (MOF)\footnote{http://www.mancoosi.org/misc-2011/criteria/ accessed 6/3/2012}, is used as the language to define the evolution preference order.
...
A CUDF document is a single plain text file, that represents all information necessary to define an evolutionary problem, as defined in section \ref{formal.step}.
...
This decision allows other component models, like Red Hat Package manager and OSGi, which allow the installation of multiple versions, to be modeled in CUDF.

%%%Upgrading with multiple packages
A request to upgrade a component is more complex than the other requests.
This complexity may be attributed to CUDF allowing having multiple versions of the same component installed.
...
then $a_1$ must be removed, and either $a_2$ can remain, or $a_2$ can be removed and a higher version of a component with name $n$ can be installed.

Given an upgrade request, no changes may be necessarily made to the system.
For example, if component with name $n$ are requested to be upgraded, and the only component with name $n$ installed is $a_1$, then the system does not have to change to satisfy the request.
...
The package stanzas are used to define to the set of components that exist at time $t$, $\mathbb{C}_t$, the component system $\alpha_{t-1}$, and the component relationship constraints $\omega_t$. 
The request stanza is used to define the user request $\delta_t$. 
A CUDF document then defines an evolution problem.

\begin{defs}
A \textbf{CUDF stanza} is a set of properties, where each property is a tuple of key/value, i.e. $stanza = \{\langle $\verb+key+, \verb+value+$\rangle\}$.
\end{defs}

\begin{defs}
A \textbf{CUDF document} is a preamble $preamble$, a set of package descriptions $PD$, and a request $request$, i.e. $CUDF = \langle preamble, PD, request \rangle$.
\end{defs} 
Each 
...
The set of components at time $t$, $\mathbb{C}_t$, equals the set of converted package descriptions, i.e. $\mathbb{C}_t = [ pd2c(pd) \mid pd \in PD]$.
...
The set of components in the component system (denoted as $\alpha_{t-1}$) is defined using the key \verb+installed+,
such that, $c \in \alpha_{t-1}$ iff  $c.$\verb+installed+ equals \verb+true+.
...
A feature is then a pair $(n,V)$ where all features are in the set $F$, $n$ is the name of the feature, 
and $V$ is a subset of positive integers representing versions, i.e $V \subseteq \mathbb{N}$.
...
The component system $\alpha_{t-1}$, is used to create the set of keep constraints $\bigcup \limits_{c \in \alpha_{t-1}} keep(c)$.
...
\end{itemize}
The order of these ranks over $\leq_L$ is then $rank(\alpha_{t-1},\alpha_t^_3) \leq_L rank(\alpha_{t-1},\alpha_t^_2) \leq_L rank_L(\alpha_{t-1},\alpha_t^_1)$.
