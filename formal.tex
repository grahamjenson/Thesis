...
CM[label=<<TABLE BORDER="0" CELLBORDER="1" CELLSPACING="5"><TR><TD WIDTH="150">CUDF<BR/>MOF</TD></TR></TABLE>> shape=none];
...
An instance of \modelname will then contain: 
\begin{itemize}
...
\subsection{CUDF*}
CUDF* is an extension and combination of the CUDF and MOF models in order to define a single document that can be mapped to a instance of \modelname.
The notable differences between CUDF and CUDF* are:
\begin{itemize}
  \item there can be multiple requests.
  \item each request stanza must define a time, and MOF criteria string, where the time maps to $t_i$, the stanza maps to $\delta_{t_i}$, and a MOF criteria maps to $\prec_{\alpha_{t_{i-1}}}$.
...
\end{itemize}
The final difference exists to create the sets of components $\mathbb{C}_{t_i}$ by including all components mapped from package descriptions that existed at (or before) time $t_i$.
...
Time $t_0$ is defined in the preamble, such that $t_0 = 100$.
Times $t_1$ and $t_2$ are defined in the requests as $t_1 = 200$, and $t_2 = 300$.

The set of components $C_{t_0}$ is the set of components that existed at time $t_0$, $C_{t_0} = \{syslib_1\}$
The set of components $C_{t_1}$ is the set of components that existed at time $t_1$, $C_{t_1} = \{syslib_1, syslib_2, textEditor_1, spellChecker_1\}$
The set of components $C_{t_2}$ is the set of components that existed at time $t_2$, $C_{t_2} = \{syslib_1, syslib_2, textEditor_1, spellChecker_1, tpspeller_1\}$

The request $\delta_{t_1}$ is the constraint set $\{textEditor_1\}$.
The request $\delta_{t_2}$ is the constraint set $\{tpspeller_1\}$.

The set of system constraints $\omega_{t_1}$ is the constraint set $\{ textEditor_1 \rightarrow spellChecker_1$,
$textEditor_1 \rightarrow syslib_2$, 
$syslib_2 \rightarrow \neg syslib_1\}$.

The set of system constraints $\omega_{t_2}$ is the constraint set $\{textEditor_1 \rightarrow spellChecker_1 \vee tpspeller_1$,
$textEditor_1 \rightarrow syslib_2$, 
$syslib_2 \rightarrow \neg syslib_1\}$.

The evolution preference order $\prec_{\alpha_{t_0}}$ is defined from the $\langle rank^{L}_{\alpha_{t_{0}}}, \leq_L \rangle$ criterion mapped from the MOF criteria string \texttt{-change,-size}.
The evolution preference order $\prec_{\alpha_{t_1}}$ is defined from the $\langle rank^{L}_{\alpha_{t_{1}}}, \leq_L \rangle$ criterion mapped from the MOF criteria string \texttt{-size,-change}.

The initial component system $\alpha_{t_0}$ is $\{syslib_1\}$.

$\alpha_{t_1} = \{syslib_2, textEditor_1, spellChecker_1\}$.


\begin{itemize}
  \item $\alpha_{t_2}^2 = \{syslib_2, textEditor_1, tpspeller_1\}$,
  \item $\alpha_{t_2}^3 = \{syslib_2, textEditor_1, spellChecker_1, tpspeller_1\}$
\end{itemize}


\begin{itemize}
  \item $rank^{L}_{\alpha_{t_{1}}}(\alpha_{t_2}^1 ) = r_1 = (2, 2)$
  \item $rank^{L}_{\alpha_{t_{1}}}(\alpha_{t_2}^2 ) = r_2 = (3, 1)$
\end{itemize}

$\alpha_{t_2}$ is therefore $\{syslib_2, textEditor_1, tpspeller_1\}$
