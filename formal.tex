...
This chapter describes the model (\modelname) and the syntax (CUDF*) of a document used to describe the a component system's evolution. 
These were developed to be used to simulate the evolution of a component system.
They also enable the discussion of CSE as they define terms and concepts used in this domain.

The developed model is called \modelname (\textbf{Co}mponent \textbf{Sy}stem \textbf{E}volution).
\modelname aims for the qualities described in \citep{Selic2003}:
\begin{itemize}
...
\end{itemize}
\modelname describes CSE as a series of evolution steps,
where each step is in response to a user's request to change the system. 

The Common Upgradeability Description Format (CUDF) \citep{treinen2009common} 
and the Mancoosi Optimisation Format (MOF) \citep{abate2011} have been used previously to describe component system evolution.
A CUDF document and a MOF string can be used to describe a single evolution step.
CUDF and MOF are therefore insufficient to describe a \modelname instance which may consist of multiple evolution steps.
The CUDF* syntax was developed to describe a \modelname instance.
CUDF* is a combination and extension of the CUDF and MOF models.
An instance of CUDF* (known as a CUDF* document) can be parsed to create an instance of \modelname.
...
The CUDF and MOF models are then presented in section \ref{formal.cudf}, and their extension to CUDF* is presented in section \ref{formal.cudf*}.
The description of parsing a CUDF* document to an instance of \modelname is presented in appendix \ref{apx.cudf}.
...
\subsubsection{Prior User Requests}
\begin{obs}
...
\section{CUDF* concrete syntax for \modelname}
\label{formal.cudf}
For \modelname to be practically used to study CSE, a concrete syntax to describe CSE problems and store results is required.
...
A CUDF document can be parsed to create a set of components $\mathbb{C}_{t_1}$, the user request $\delta_{t_1}$, system constraints $\omega_{t_1}$,
...
Together CUDF and MOF provide only enough information to describe the evolutionary step from system  $\alpha_{t_{0}}$ to  $\alpha_{t_{1}}$.
...
\begin{itemize}
  ...
  \item sets of user requests $\delta_{t_1},\delta_{t_2}$
...
\section{Summary}
In this chapter the formal model \modelname is presented to describe the process of component system evolution.
How to compose multiple criteria into a single evolution preference order was then discussed.
The CUDF and MOF were then presented and their parsing into a \modelname instance that represented a single evolutionary step was described.
Given multiple steps are required to express the CSE process, CUDF* was presented as an extension and combination of the CUDF and MOF models.
The mapping from a CUDF* document to a \modelname instance was then described.

The models presented in this chapter can be used to simulate and study the CSE process.
