...
In the previous chapter, component system evolution (CSE) was described as the continual alteration of the composition of components in a system by a composer or user.
Any change made during evolution must satisfy constraints to ensure that the resulting system is valid.

This chapter describes a model (\modelname) and syntax (CUDF*) used to describe CSE. 
...
\end{itemize}
\modelname describes CSE as a series of evolution steps, where each step is in response to a user's request to alter the system. 
...
However, for the purpose of this thesis, this formats are insufficient due to the fact that \modelname describes CSE as a series of steps rather than a single step.
...
In sections \ref{formal.step} and \ref{formal.opt} the \modelname model is defined and discussed.
...
Each step tries to satisfy a user's request for change and the system constraints that ensure the system is valid.
The selection problem (discussed in chapter \ref{background}) that occurs during and evolution step is also modeled.
This is when multiple component systems satisfy the user's request and the system constraints, and the best solution for the user should be selected.
...
Only certain types of constraints are required to be able to describe component system evolution.
...
\end{defs}
A valid system with respect to an evolution problem, is described as a solution to the evolution problem.
...
\begin{defs}
The set of \textbf{user's requests} $\Delta$ is the set of all constraints, where each constraint in $\Delta$ is either:
\begin{itemize}
...
At any point in time, the sets of system constraints and user's request constraints are with respect to the set of components that exist at that time.
\begin{defs}
...
and the set of \textbf{user's requests w.r.t. time} is $\Delta_{t}$, where $\Delta_{t} = \{con \cap 2^{\mathbb{C}_{t}} \mid con \in \Delta\}$
\end{defs}

\begin{defs}
Given a time $t$, a \textbf{user's request}, $\delta_{t}$, is a finite set of user's requests at time $t$, i.e. $\delta_{t} \subseteq_{finite} \Delta_{t}$;
...
An evolution problem, $EP$, can be defined by combining a user's request $\delta_{t}$ and a set of system constraints $\omega_{t}$, such that $EP = \delta_{t} \cup \omega_{t}$.

\subsubsection{Example}
Consider the system constraint $con_1$ that is defined as $a \rightarrow b \vee d$, and the user's request constraint $con_2$ defined as $a$ (a singleton installation constraint).
...
The user's request constraint $con_2' \in \Delta_{t}$, is such that $con_2' = con_2 \cap 2^{\mathbb{C}_{t}} = \{\{a\},\{a,b\}\}$.

Assume a set of system constraints $\omega_{t} = \{con_1'\}$, and a user's request constraints $\delta_{t} = \{con_2'\}$.
...
\label{formal.evo}
A component system is evolved from one system to another to satisfy an evolution problem consisting of a user's request and a set of system constraints.
...
a user's request $\delta_{t_i}$, and a set of system constraints $\omega_{t_i}$;
...
If an evolution problem is unsatisfiable it could be possible to return a component system that maximises the number of satisfied constraints in the user's request.
This approach, however, is difficult because the intentions of a user's request are impossible to determine.
...
Given the user's request, this question cannot be answered as the user may of only required both $a$ and $b$, and only installing one will not satisfy the user.
...
sets of components $C_{t_1},\ldots,C_{t_n}$, a series of user's requests $\delta_{t_1},\ldots,\delta_{t_n}$, a series of system constraints $\omega_{t_1},\ldots,\omega_{t_n}$,
...
\begin{itemize}
  ...
  \item sets of user's requests $\delta_{t_1},\ldots,\delta_{t_n}$.
...
\subsubsection{Prior User's Requests}
\begin{obs}
...
This is similar, as a set of previous user's requests should try to remain satisfied with the component system, as the user requests to change the system.    
...
\begin{itemize}
  ...
  \item a series of user's requests be $\delta_1 = \{a\}$ and $ \delta_2 = \{c\}$
...
This counter-intuitive result shows that the incomplete knowledge of future user's requests may effect the optimality of the evolved component system.

\subsection{Optimal Component System Selection}
\label{formal.opt}
...
All systems presented in chapter \ref{background} such as Eclipse P2, apt-get, aptitude, and MPM, define multiple criteria that express the preference in their systems.
...
It is the selected mechanism to define the preference order in this thesis, and is defined in the following section.

\subsubsection{Criteria and Lexicographic Composition}
...
This syntax is defined in CUDF* which extends and combines the Common Upgradeability Description Format (CUDF)\footnote{This specification was previously discussed in section \ref{background.models}, with an example shown in figure \ref{CUDFmetadata}.
}
...
A CUDF document can be parsed to create a set of components $\mathbb{C}_{t_1}$, the user's request $\delta_{t_1}$, system constraints $\omega_{t_1}$,
and a component system $\alpha_{t_{0}}$.
A MOF criteria string can be parsed to create an evolution preference order ($\prec_{\alpha_{t_0}}$).
Together CUDF and MOF provide only enough information to describe the evolution step from system  $\alpha_{t_{0}}$ to  $\alpha_{t_{1}}$.

To provide enough information to fully describe the CSE process, as defined by the $\epsilon^*$ function,
CUDF and MOF are combined and extended into the CUDF* model.
A CUDF* document can be parsed to create a \modelname instance, 
which includes:
\begin{itemize}
  \item a series of times $t_0,\ldots,t_n$
  \item sets of components $\mathbb{C}_{t_0},\ldots,\mathbb{C}_{t_n}$
  ...
  \item initial system $\alpha_{t_0}$
\end{itemize}
This instance can then be used to simulate the evolution of a component system by finding the systems $\alpha_{t_1}$,\ldots,$\alpha_{t_n}$.

In this section the CUDF and MOF specifications are described.
The CUDF* model is then described with a brief description of how CUDF* is parsed to create a \modelname instance (for the full description please refer to appendix \ref{apx.cudf}).
...
One such contribution was to hold the Mancoosi International Solver Competition (MISC), which compared solvers of evolution problems.
MISC was held to encourage researchers and practitioners into researching the component system evolution problem.

An initial problem faced by Mancoosi when organising MISC was that no common format existed to describe evolution problems.
The design of such a format should follow the principles \citep{treinen2009common}:
...
The result of these design goals is the CUDF specification \citep{treinen2009common}, described in the following sections.

\subsubsection{CUDF Document}
...
Some aspects of CUDF are not taken into account (e.g. the formal type system) as a complete redefinition of the original specification is not the goal of this section.
...
\begin{itemize}
  ...
  A caveat to this is that a component cannot conflict with itself.
\end{itemize}
...
This stanza is mapped to the user's request $\delta_{t_1}$.
The request stanza contains three properties that define the user's request to change the system.
...
a set of system constraint constraints $\omega_{t_1}$, 
and a set change request constraints $\delta_{t_1}$.
...
The set of components in the CUDF document are defined from the package stanzas,
...
$\alpha_{t_{i-1}} = \{ syslib_1 \}$.

\subsubsection{System Constraints}
%%%Describe the depends constraints (including features and inequalities)
The set of system constraints $\omega_{t_1}$ is defined by the \texttt{depends} property of $textEditor_1$, and the \texttt{conflicts} property of $syslib_2$.
...
The set of system constraints $\omega_{t_1}$ is defined as
...
The user's request $\delta_{t_1}$ is defined by the request stanza's install property.
...
CUDF does not include the criteria used to select a potential solution.
Mancoosi defined this criteria in a separate format which is called in this research the Mancoosi Optimisation Format (MOF).
This format, like CUDF, was designed for MISC.

MOF was designed to be passed on the command line, this meant that it must be expressible in a single line of human writable text.
This results in the relatively simple specifications as opposed to other more complicated specification like \citep{Treinen2009}.

A MOF string is a list of criteria separated by the delimiter ``\texttt{,}''.
Each criterion in this list can be mapped to a criterion $\langle rank_{\alpha}, \leq \rangle$.
The entire string is defined lexicographically into a single criterion $\langle  rank^L_{\alpha}, \leq_L \rangle$,
with the $\oplus$ operator. 
This criterion can be used to create the optimality order $\prec_{\alpha}$ as described in definition \ref{formal.defcrittooptimal}.

\subsubsection{Example}
\label{formal.mofexample}
An example of how MOF is used to define an evolution order is given using the example presented in section \ref{formal.example}.

In this example, the component system $\alpha_{t_{0}}$ was  $\{syslib_1\}$, and there were three possible solutions:
\begin{itemize}
  \item $\alpha_{t_1}^1 = \{syslib_2, textEditor_1, spellChecker_1\}$,
  \item $\alpha_{t_1}^2 = \{syslib_2, textEditor_1, tpspeller_1\}$,
  \item $\alpha_{t_1}^3 = \{syslib_2, textEditor_1, spellChecker_1, tpspeller_1\}$
...
For this, first two criteria are defined as $crit_{change} = \langle rank^{change}_{\alpha_{t_{0}}}, \leq_{change} \rangle$ and $crit_{size} = \langle rank^{size}_{\alpha_{t_{0}}}, \leq_{size} \rangle$.

The criterion $crit_{change}$ is is mapped from the string \verb+-change+.
$rank^{change}_{\alpha_{t_{0}}}$ is defined as the cardinality of the symmetric difference between systems, i.e. $rank^{change}_{\alpha_{t_{0}}}(\beta) = |\alpha_{t_{0}} \Delta \beta|$.
The order $\leq_{change}$ is defined as $>$ (less change is better) over natural numbers. 

The criterion $crit_{size}$ is mapped from the string \verb+-size+.
$rank^{size}_{\alpha_{t_{0}}}$ is defined as the sum of the property \verb+size+ of all components in the  proposed system, i.e. $rank^{size}_{\alpha_{t_{0}}}(\beta) = \sum_{c \in \beta} c$.\verb+size+.
The order $\leq_{size}$ is defined as $>$ (smaller system is better) over natural numbers. 

For the MOF string \verb+-change,-size+, the resulting criteria would be $crit_{change} \oplus crit_{size} = \langle rank^{L}_{\alpha_{t_{0}}}, \leq_L \rangle$, as shown in definition \ref{formal.defcritlex}.
...
This makes $s_1$ an optimal solution, as no other solutions $rank^{L}_{\alpha_{t_{0}}}$ is greater w.r.t. $\leq_L$. 

\subsection{CUDF*}
\label{formal.cudfstar}
CUDF* is an extension and combination of the CUDF and MOF models created in order to define a single document that can be mapped to an instance of \modelname.
The differences between CUDF and CUDF* are:
\begin{itemize}
  \item each package description stanza must defines a time that the component first existed.
  \item the preamble defines the time $t_0$. 
  \item there can be multiple requests stanza's, where each request stanza must define a time $_i$, and MOF criteria string. 
  The stanza maps to request $\delta_{t_i}$, and a MOF criteria maps to $\prec_{\alpha_{t_{i-1}}}$.
\end{itemize}
...
For a full description of the process of parsing a CUDF* document to a \modelname instance, please refer to appendix \ref{apx.cudf}.
...
This CUDF* document can be parsed to an instance of \modelname,
which includes:
\begin{itemize}
  \item a series of times $t_0,t_1,t_2$
  \item sets of components $\mathbb{C}_{t_0},\mathbb{C}_{t_1},\mathbb{C}_{t_2}$
  \item sets of user's requests $\delta_{t_1},\delta_{t_2}$
  \item sets of system constraints $\omega_{t_1},\omega_{t_2}$
  \item evolution preference orders $\prec_{\alpha_{t_0}}, \prec_{\alpha_{t_{1}}}$
  \item initial system $\alpha_{t_0}$
\end{itemize}

The initial component system $\alpha_{t_0}$ is $\{syslib_1\}$ (the same from the previous example).
...
\begin{itemize}
  ...
  \item $\delta_{t_2}$ $=$ $\{tpspeller_1\}$.
\end{itemize}

The preference orders are extracted from the MOF criteria in each request.
As described in section \ref{formal.mofexample} the MOF criterion \texttt{-size} is mapped to $crit_{size}$, and \texttt{-changed} maps to $crit_{change}$.  
\begin{itemize}
  \item the criteria $crit_{change} \oplus crit_{size}$ is used to define $\prec_{\alpha_{t_0}}$
  \item the criteria $crit_{size} \oplus crit_{change}$ is used to define $\prec_{\alpha_{t_1}}$
\end{itemize}

Consider the evolution from $\alpha_{t_0}$ to $\alpha_{t_1}$.
...
Consider the evolution from $\alpha_{t_1}$ to $\alpha_{t_2}$.
...
CUDF* documents can then be used to describe and study the CSE process. 

\section{Summary}
This chapter described the model \modelname and the syntax CUDF* which are used to represent component system evolution.
Also presented was an example of parsing a CUDF* document to create an instance of \modelname to describe the evolution of a component system.
The full description of parsing a CUDF* document to a \modelname instance is given in appendix \ref{apx.cudf}.
