...
Each step tries to satisfy a user request for change and the system constraints that ensure the system is valid.
The selection problem (discussed in chapter \ref{background}) that occurs during and evolution step is also modeled.
This is when multiple component systems satisfy the user request and the system constraints, and the best solution for the user (given some criteria) should be selected.
...
\label{formal.evo}
A component system is evolved from one system to another to satisfy an evolution problem consisting of a user request and a set of system constraints.
...
The non-strict evolution preference order $\preceq_{\alpha}$ can be defined such that $\beta \prec_{\alpha} \beta'$ if and only if $(\beta \preceq_{\alpha} \beta'$ and $\beta \neq \beta')$. 
 other telescopes were able to study the planet by analyzing how the light from 55 Cancri changed as the planet passed in front of the star.
...
\begin{itemize}
  ...
  \item initial system $\alpha_{t_0}$
\end{itemize}
This instance can then be resolved to find the component systems $\alpha_{t_1}$,\ldots,$\alpha_{t_n}$.
...
This section briefly describes the CUDF format.
...
The user request $\delta_{t_1}$ is defined by the request stanza's install property.
...
By going through the constraints this problem can be resolved.
...
Like CUDF, this formast was designed for MISC.
...
The criterion $crit_{change}$ is is mapped from the MOF string \verb+-change+.
...
The criterion $crit_{size}$ is mapped from the MOF string \verb+-size+.
...
\begin{itemize}
  \item each package description stanza must define the time that the component first existed.
  \item the preamble defines the time $t_0$. 
  \item there can be multiple requests stanza's, where each request stanza must define a time $t_i$, and MOF criteria string
