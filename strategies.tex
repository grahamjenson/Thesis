...
Given the users main tool to evolve a system is CDR, the functionality that CDR provides the scope of actions a user can take. 
These action are limited to a user to requesting a component be added, removed or upgraded in their system.
A user may also choose the criteria that is used to select the optimal system that satisfies the request.
When these actions are taken is the final part of the strategy, as this provide s

%%%Why the user does it
Even with the limited set of actions defined in CDR, a great deal of strategies can be defined.
