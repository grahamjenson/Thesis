...
%%%CDR is more complex though, as there are typically many solutions that can be selected from, each with different properties; it is an optimisation problem making it NP-Hard
...
%%%We extend this definition o CDR to include optimisation through some measurement of improvement over the previous system\\
...
%%%Here we easily reduce the problem to SAT, then showing that it is not a simple SAT problem, because of the conflict constraints, it is NP-Complete, this has been done many times before

\section{Optimisation}
%%%this section is a reduction (remove lattices, leave order theory) of our APSEC 2011 paper\\
%%%CDR can return potentially enormous amount of solutions, each has different properties that may make it more or less desirable than another, we need criteria to find an optimial solution
...
%%%This problem is however multi criteria as users will likely have more than one criteria to judge a solution by
...
%%%A naive solution is to map all criteria to a real number, then aggregate them through addition multiplcation with weights
...
%%%We have opted for a formal approach of using orders over partially ordered sets, where any sub set is bounded, which is a complete lattice
...
%%%Complete Lattices are used in this framework, have been used in other frameworks before
...
%%%The domiain is our value of the criteria
...
%%%We define a ranking system and function, which returns a value in the complete lattice domain that represents the ranking of the criteria
An optimal solution can now be defined through a ranking system with respect to complete lattices.

%%%A ranking system is a lattice a finite domain of variables, their domain
\begin{defs}
...
%%%Just discuss the related yet incompatiable work C-semirings, and how it is incompatable and influential.
